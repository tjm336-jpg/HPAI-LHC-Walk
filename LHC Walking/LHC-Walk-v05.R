...
HPAI_dyn                      <- function(time, state, parameters){
  ...
  with(as.list(c(state,parameters)),{
    ...
    #===============#
    dS_Gentoo = 
      ifelse(time >= timevar$gentoo_breed_start & time <= timevar$gentoo_breed_finish, 10 * muH_Gentoo * (S_Gentoo + I_Gentoo + R_Gentoo), 0) - #Gentoo birth rate
...
for(i in 1:num_samples){
  ...
  output                      <- run_Model(timevar, popvar, Initial_values, HPAIdyn, parameters, time1, time2, spec$R0)
