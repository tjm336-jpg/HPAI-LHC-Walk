...
#                                                 Model Grouping:
#=======================================================================================================================#
I_Gentoo_Peaks    <- function(Valarray, dfMaster, compartments, outputLoc){
  numSamples   <- dim(Valarray)[3]
  Time         <- as.numeric(colnames(Valarray))
  popvar       <- generate_popvar()
  situationLoc <- paste0(outputLoc, "/IGentooPeaks")
  dir.create(situationLoc)
  
  for (i in 1:numSamples){
    run      <- Valarray[,,i]
    out      <- as.data.frame(t(run))
    out$Time <- Time
    ...
    } else if (length(tI_G) == 2){
      dfMaster$Cluster[i] = 2
    } else if (length(tI_G) == 3){
      dfMaster$Cluster[i] = 3
    } else if (length(tI_G) >= 4){
      dfMaster$Cluster[i] = 4
    }
  }
  for (i in 1:4){
    res   <- vardistplots_kl(dfMaster, i)
    p     <- res$plot
    ggsave(paste0(situationLoc, "/InputDistrib_Cluster", i, ".png"), p, width = 14, height = 8)
    plots <- outdistgraph(Valarray, dfMaster$Cluster, i, compartments, popvar)
    p     <- plot_grid(plotlist = plots, ncol = length(compartments)/3, align = "hv")
    ggsave(paste0(situationLoc, "/CompartmentGraphs_Cluster_", i,".png"), p, width = 15, height = 10)  
  }
}
I_Predators_Peaks <- function(Valarray, dfMaster, compartments, outputLoc){
  numSamples   <- dim(Valarray)[3]
  Time         <- as.numeric(colnames(Valarray))
  popvar       <- generate_popvar()
  situationLoc <- paste0(outputLoc, "/IPredatorsPeaks")
  dir.create(situationLoc)
...
I_Albatross_Peaks <- function(Valarray, dfMaster, compartments, outputLoc){
  numSamples   <- dim(Valarray)[3]
  Time         <- as.numeric(colnames(Valarray))
  popvar       <- generate_popvar()
  situationLoc <- paste0(outputLoc, "/IAlbatrossPeaks")
  dir.create(situationLoc)
