...
check_initVal    <- FALSE
check_season     <- TRUE
check_eventdata1 <- FALSE
check_eventdata2 <- FALSE
...
save_output      <- FALSE
gentoo_output    <- TRUE
predators_output <- TRUE
albatross_output <- TRUE
...
prev_infection_preds  <- 0.01                                       # Rate at which arriving predators are infected
...
preds_inf_dur         <- 7                                          # Duration of infection in Predator species could be anywhere from 1-10 [Source: 6]
gamma_preds           <- 1 / preds_inf_dur                          # Calculate the recovery rate for Predator species 
preds_inf_mort        <- 0                                          # Estimate without a basis
muI_Preds             <- preds_inf_mort                             # Calculation of the infected predator species death rate
...
time1                 <- seq(from = 0, to = t_departure, by = 1)       # Set the front part of the time sequence
time1                 <- round(time1, 3)                               # Round all the times
time2                 <- seq(from = t_departure, to = horizon, by = 1) # Set the back part of the time sequence
...
#                                                Event Functions:
#=======================================================================================================================#
create_eventdata1  <- function(t_arrival, mig_day_arrival, prev_infection_preds, preds_migrate_in, prev_infection_albs, albatross_migrate_in){
  # Create a list of the number of days where these arrival will happen
  mig_arrival_times     <- seq(
    from       = t_arrival, 
    to         = t_arrival + mig_day_arrival -1, 
    length.out = mig_day_arrival) 
  
  # Calculate the number of birds that arrive on each day
  pred_S_arrival <- (1 - prev_infection_preds) * preds_migrate_in / mig_day_arrival
  pred_I_arrival <-  prev_infection_preds       * preds_migrate_in / mig_day_arrival
  alb_S_arrival  <- (1 - prev_infection_albs)  * albatross_migrate_in / mig_day_arrival
  alb_I_arrival  <-  prev_infection_albs        * albatross_migrate_in / mig_day_arrival
...
create_eventdata2  <- function(t_departure, mig_day_departure, state){
  mig_departure_times   <- seq(
    from       = t_departure,
    to         = t_departure + mig_day_departure - 1,
    length.out = mig_day_departure)

  # Calculate the number of birds that depart on each day
  pred_S_departure <- - (0.9 / mig_day_departure) * state$S_Preds
  pred_I_departure <- - (0.9 / mig_day_departure) * state$I_Preds
  pred_R_departure <- - (0.9 / mig_day_departure) * state$R_Preds
  albs_S_departure <- - (1.0 / mig_day_departure) * state$S_Albatross
  albs_I_departure <- - (1.0 / mig_day_departure) * state$I_Albatross
  albs_R_departure <- - (1.0 / mig_day_departure) * state$R_Albatross
...
eventdata1 <- create_eventdata1(t_arrival, mig_day_arrival, prev_infection_preds, preds_migrate_in, prev_infection_albs, albatross_migrate_in)

# Validation of no errors in eventdata1
if(check_eventdata1 == TRUE){
  print(eventdata1)
}
...
eventdata2 <- create_eventdata2(t_departure, mig_day_departure, out_pre[ nrow(out_pre), ])

# Validation of no errors in eventdata2
if(check_eventdata2 == TRUE){
  print(eventdata2)
}
...
if(gentoo_output == TRUE){
  graph_Gentoo(output)
}
if(predators_output == TRUE){
  graph_Predators(output)
}
if(albatross_output == TRUE){
